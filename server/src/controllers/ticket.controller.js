...
export const bookTicket = async (req, res) => {
  try {
    ...
    // decrement ticket first
    const event = await Event.findOneAndUpdate(
      { _id: eventId, isActive: true, availableTickets: { $gt: 0 } },
    ...
    try {
      ...
      // rollback availableTickets if ticket create fails
...
export const getUserTickets = async (req, res) => {
  try {
    ...
    const tickets = await Ticket.find(filter)
      .populate({ path: "event", select: "title date time location image ticketPrice" })
